...
async function loadConfig() {
  try {
    ...
    if (r.ok) { const c = await r.json(); await chrome.storage.local.set({ cfg: c }); return c; }
  } catch {}
  const { cfg } = await chrome.storage.local.get('cfg');         // прошлый удачный
  if (cfg) return cfg;
  return fetch(chrome.runtime.getURL('config.default.json')).then((r) => r.json()); // вшитый запасной
}
...
async function fetchNames(cfg) {
  for (const src of cfg.name_sources || []) {
    try {
      ...
      await chrome.storage.local.set({ names: j, names_ts: Date.now() });
      return j;
    } catch {}
  }
  const { names } = await chrome.storage.local.get('names');     // офлайн-кэш
  return names || {};
}
...
async function main() {
  ...
  $('#home').addEventListener('click', (e) => { e.preventDefault(); location.replace(chrome.runtime.getURL('app/search.html')); });
