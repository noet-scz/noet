// noet — расширение (Chromium MV3, Vivaldi/Chrome/Brave/Edge).
//
// Ставит PAC: все хосты *.nt идут на локальный резолвер noet (127.0.0.1:8090),
...
async function setState(on) {
  ...
  chrome.action.setTitle({ title: `noet: ${on ? 'включено' : 'выключено'} (клик: переключить)` });
}
